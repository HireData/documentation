...
_Most feedback requests ask twice. The email asks you to give feedback, the form asks the actual question, and half the people you asked never get as far as the question. A Form block collapses that into one step. The rating sits in the email as a row of buttons, and the click that answers it is the same click that opens the form._
...
Open **Emails** from **Settings**, click **New Email**, and select **Blank Email** or one of the supplied templates. Set **Brand** in the top right first. It restyles the whole email, including the button colours, so choosing it early saves you checking the contrast twice. See [Setting up an email template](/apps/messaging/emails/setting-up-an-email-template-in-hiredata).
...
Click the **+** in the section's **Blocks** list and select **Form**, then set the two selects at the top of its panel.
