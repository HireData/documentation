...
The email service behind the Email app creates the DNS records for your domain. As long as the Email app is not installed, there is no email service connected to your account yet, so HireData cannot generate or verify DNS records. The **Verify DNS** button on the **Domains** page then returns a 404 error or no response.
...
Now that the Email app is installed, return to the [**Domains**](https://app.hiredata.com/settings/account/domains) page in your settings. You can now generate the DNS records for your domain.
...
<Warning>
  DNS changes can take from a few minutes up to 24–48 hours to propagate, depending on your provider. If verification doesn't succeed right away, allow some time and try again.
</Warning>
