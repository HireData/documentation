...
The editor asks for the following.
...
| **What should the AI do?** | The mode: `Custom`, `Extract`, `Summarize`, `Classify`, `Write`, `Translate`, or `Analyze`. `Custom` lets you write the whole prompt yourself; the others come pre-shaped for their job. |
...
- **The Variables tab.** Choose **Add Field** and attach the fields the variable needs. This is the clearer option, and it makes the dependencies visible to anyone who opens the variable later.
- **Inline in the prompt.** Write `{{ candidate.first_name }}` directly in the instruction and it is substituted before the instruction is sent.

Either way, the attached and referenced fields become the variable's inputs, which is also what decides when it regenerates.
