...
Type `{{` anywhere in the **Input** box and a picker appears, listing every variable available to this template with its key and its type. Keep typing to filter, then choose one to insert it. It appears as a badge reading the field's display name, such as **Recipient: First Name**. Hover the badge for its value and any modifiers, and see [Variables](/variables/introduction) for the badge states.
...
Each badge is replaced with that recipient's value before the instruction reaches the model, so for Sofia the model is asked for a wish _for Sofia_ rather than for `{{recipient.first_name}}`. A badge that has turned orange reads **Variable is not found** and reaches the model as the raw key, so fix it before you save.
