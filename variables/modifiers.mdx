...
description: "Transform variable values: change case, format dates and numbers, build conditions, and more"
---

_Modifiers change how a [variable](/variables/introduction) value appears when your content is sent. Add one with a pipe (`|`) after the field._
...
```
{{ candidate.first_name | uppercase }}
```

**Chaining.** Add more pipes to apply modifiers in order, from left to right. Each one acts on the result of the previous:

```
{{ candidate.first_name | lowercase | capitalise }}
```

**Arguments.** Some modifiers take extra values after a colon, separated by commas:

```
{{ job.title | truncate_chars: 20 }}
...
| `camel_case` | Joins words in camelCase | `john doe` → `johnDoe` |
| `pascal_case` | Joins words in PascalCase | `john doe` → `JohnDoe` |
| `snake_case` | Joins words with underscores | `john doe` → `john_doe` |
| `kebab_case` | Joins words with hyphens | `john doe` → `john-doe` |
| `dot_case` | Joins words with dots | `john doe` → `john.doe` |
| `slug` | Makes a URL-friendly slug | `John Doe!` → `john-doe` |
...
| `highlight: term` | Wraps matches in a highlight | highlights `term` in the text |
| `ascii` (`transliterate`) | Converts accented letters to plain ASCII | `José` → `Jose` |
| `escape_html` | Makes text safe to show as HTML | `<b>` → `&lt;b&gt;` |
| `strip_html` | Removes HTML tags | `<b>hi</b>` → `hi` |
| `markdown_to_html` | Renders Markdown as HTML | `**hi**` → `<strong>hi</strong>` |
| `sprintf: format` (`format`) | Formats with a pattern | `sprintf: "%05.2f"` on `3.1` → `03.10` |
...
| `modulo: n` | Remainder after division | `modulo: 2` on `5` → `1` |
| `min: n` | Caps at a minimum | `min: 0` on `-3` → `0` |
| `max: n` | Caps at a maximum | `max: 100` on `120` → `100` |
...
| `sign` | Returns -1, 0, or 1 | `-7` → `-1` |
| `pow: exponent` (`power`) | Raises to a power | `pow: 2` on `5` → `25` |
| `sqrt` (`square_root`) | Square root | `16` → `4` |
| `to_fixed_number: decimals` (`to_fixed`) | Fixed number of decimals | `to_fixed_number: 2` on `3.1` → `3.10` |
...
| `to_timestamp` (`unix`, `timestamp`) | Unix timestamp | |
| `iso8601` (`iso_date`) | ISO-8601 date string | `2026-03-14T09:00:00+00:00` |
...
| `get: key` | Reads a value by key or path | `get: "skills"` |
| `pluck: key` (`column`) | Takes one key from each item | `pluck: "name"` |
| `keys` | The keys of the data | |
| `values` | The values of the data | |
| `parse_json` | Reads a JSON value so other list modifiers can use it | |
| `to_json` | Converts a value to JSON text | |
