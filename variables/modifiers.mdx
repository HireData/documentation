...
A variable with modifiers still reads as its display name in the editor. The badge adds a wand icon, and a count once you chain two or more. Hover the badge and the popover shows the whole pipeline.
...
- **Input** is the value the field holds right now, with its type icon.
- **One card per modifier**, in order, named in plain language. `lowercase` reads **Lowercase**, `capitalise` reads **Capitalise words**, `truncate_chars` reads **Limit characters**, and `default` reads **Use a fallback**. Arguments appear under the name in code, so `default: "there"` shows `"there"` on its own line.
- **Output** is the value after every step, with the icon of the type the chain produces.

The preview runs the supported modifiers on the current value. A modifier the preview cannot run keeps its card and its arguments and shows an info icon with the note **This modifier cannot be previewed.** Later cards show **This preview depends on a value or earlier step that cannot be evaluated.** The modifier still runs when the content is sent.

The popover applies the modifiers for you to read. It does not change the expression. To edit it, place the cursor after the badge and press Backspace. See [Editing a badge](/variables/introduction#editing-a-badge).
...
In the popover the fallback gets a card of its own reading **Use a fallback** with the fallback text under it. When the field is empty, the **Input** card reads **No value provided** and the **Output** card shows the fallback. A record's **Forms** tab shows this for the record you have open, so you can check which vacancies would fall back.

## Literals

A quoted text, a number, or `true` or `false` can stand in place of a field. The braces still make it a badge, and modifiers apply to it in the same way:

```text
{{ "text" | uppercase }} → TEXT
```

The badge shows the literal on a tinted background with the icon of its type: a text icon for `"text"`, a number icon for `42`, a checkbox icon for `true`. Its popover reads **Input** `text`, **Uppercase**, **Output** `TEXT`.

<img src="/images/email-builder/22-literal-popover.png" alt="A tinted badge reading text with a text-type icon and a wand icon, and its popover with Input text, Uppercase, and Output TEXT" width="616" />

A literal without modifiers, such as `{{ 42 }}`, sends the value as written and its popover shows it under **Value**. Spaces inside the braces are optional.
