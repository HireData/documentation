...
Every variable above **reads** a value that already exists. When you need a sentence that has to be written fresh for each recipient — a personal opener, a non-generic birthday line — use an [AI variable](/variables/ai-variables) instead.

Use them for prose, never for facts. Dates, salaries, and contract terms belong in a normal variable.