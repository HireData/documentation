...
<Note>
  **On the canvas.** A Message task shows a **Task** badge, the task name, and what it will send. Send Email renders the template and lists Subject, Recipient, Sender, and Form.

  ![A Send Email card on the canvas with a rendered email preview and rows for Subject, Recipient, Sender, and Form](/images/automations/28-send-email-card.png)
</Note>
...
![A run timeline with Send Email: Cancelled, the message Invalid config., and Send Email: Run Cancelled](/images/automations/35-message-cancelled-timeline.png)
