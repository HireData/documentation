...
**Prompt** asks an AI model to do something with the run's data and returns the answer as fields for later steps. The picker describes it as "Asks an AI model to do something." The drawer is headed **Ask AI** and its footer holds **Back**, **Preview**, **Cancel**, and **Save Task**. **Preview** runs the prompt against sample data before you save.

The canvas card shows a **Task** badge, "Ask AI" followed by the intent, the prompt text, a chip per output field, and the three Advanced values.
...
- **What should the AI do?** The intent. **Custom** writes the prompt from scratch. The others frame the prompt for you: **Extract** pulls structured data out of text, **Summarize** condenses content, **Classify** puts content into predefined groups, **Write** generates or rewrites in a chosen tone, **Translate** converts between languages, **Analyze** looks for insights, sentiment, or patterns, and **Evaluate** assesses content against criteria.
...
- **Advanced**. Collapsed by default.
  - **Step name**. Replaces the automatic name in the drawer header only. The canvas card and the variable names keep the automatic name.
...
- **Expecting Step name to rename the card.** It changes the drawer header only.
