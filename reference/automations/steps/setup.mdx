...
Every automation begins with the **Start** node. Clicking it opens a drawer headed **Setup**, where you name the event that begins a run. An event that matches the settings here starts one run; an event that does not match starts nothing.
...
- **Trigger**. Which change to the object counts. For a Response: **Completed**, **Created**, **Created or Updated**, or **Updated**. The choices are listed under the heading **When something happens**.
...
- **Filter**. Conditions the event must meet before a run starts. The run page lists it in the event panel next to App, Object, and Trigger. This is the start filter. It is not a [Filter step](/reference/automations/steps/filter).

The drawer's footer holds **Back**, **Cancel**, and **Save**. Saving writes the whole automation at once. A new automation is called "Blank Automation" until you rename it, and the name is kept only after Setup has been saved once.
