...
<Note>
  **On the canvas.** The Split is a box with a **Split** badge. The branch Filters sit side by side inside it, each with its own **+** below, and the box's own **+** follows underneath.

  ![A Split box on the canvas with two Filter branches, Question 2: Answer is not empty and Question 2: Answer is empty](/images/automations/25-split-card.png)
</Note>
