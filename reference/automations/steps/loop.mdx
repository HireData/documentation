...
**Loop** repeats the steps inside it once per record. The picker describes it as "Repeats a set of actions." The drawer is headed **Loop** and opens with one option, **Custom**, described as "Loops through a selection of records." Choosing it leads to the object list, then the filter panel. The footer holds **Cancel** and **Save**, then **Back** and **Save** on the later panels.

After you save, the **Next step** picker opens for the first step inside the loop. The canvas card shows a **Loop** badge, a line such as "Retrieve Candidates from Carerix", the connection name, the filter, and a **+** inside the card for the loop's own steps.
...
- Records that cannot be retrieved. The Loop shows Cancelled with the message `data_lookup_failed`. Check the connection and the filter fields.
- A loop with no step inside it. The Loop shows Failed with the message `step_not_found`.
