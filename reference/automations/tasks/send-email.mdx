...
- **CC** and **BCC**. Click **+** to add a copy recipient. Each entry has a **Name** and an **Address**. Both take a field from the run or typed text: type it and select the **Create** option that appears under the box. Fill both before saving. **–** removes an entry. CC addresses are visible to everyone on the email; BCC addresses are not. A test send drops CC and BCC.
