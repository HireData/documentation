...
**Send Email** sends an email built from one of your email templates to one address per run. It sits under **Message** in the **Next step** drawer, marked with the HireData icon. The drawer is headed **Send Email** and its footer holds **Cancel** and **Save Task**.
...
- **Objects**. Shown when the template or its form reads from a data source. Picks the record each object refers to in this run.
...
The sent email and its delivery status. On the run page the task's card shows the email that went out; click it to read the message as the recipient saw it. The run step's **Fields** tab lists the values the task used, such as the subject, recipient, and sender.
...
A recipient address that is not a valid email address, an address on your blocklist, an address that unsubscribed, or an address that email validation marked invalid also stops the run, each with its own message in the timeline. Reasons are explained in [Run cancellation reasons](/knowledge-base/run-cancellation-reasons).
