...
2. **Check the canvas for orange badges.** A variable HireData recognises is a plain badge, with or without [modifiers](/variables/modifiers). An orange badge with a triangle icon is a key it does not recognise, and its popover reads **Variable is not found**. Fix or remove each one. See [Variables](/reference/email-builder/variables).
