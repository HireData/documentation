...
description: "Personalise a HireData email with variables: the picker, where they work in an email, adding your own, linking records, and what a recipient sees when a value is missing."
...
The email builder shows a variable as a badge carrying its label, not its value. `{{recipient.first_name}}` reads **Recipient: First Name** on the canvas, and the badge behaves as one object rather than as the characters it stands for. Hover it to see the value it holds. See [Variables](/variables/introduction) for the badge, its popover, and how to edit it.
...
Three things can happen to a variable when the email goes out, and only one of them is what you meant.
...
The builder tells you which of the three you are heading for. Hover a badge to see the value it holds, and look out for an orange badge, which is a key HireData does not recognise. Add a `default` modifier to cover the second and third cases with words of your own. [Variables](/variables/introduction) explains the badge and its popover, and [Modifiers](/variables/modifiers#fallback) explains the fallback.
...
- [Variables](/variables/introduction) covers the `{{ }}` syntax, the badge, and its popover across forms, messages, and automations as well as email.
