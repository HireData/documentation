...
description: "Send one HireData email in several languages: managing languages, what the builder offers for translation, and which language a recipient gets."
...
**Create AI translation** fills every box at once. It keeps your variable badges where they are and translates the words around them, so `Hi {{recipient.first_name}},` comes back as `Hoi {{recipient.first_name}},` rather than losing the name. Read what it wrote before you keep it, and edit any box you disagree with.
...
Click a language in the **Translations** list to reopen it. You cannot change the language, and the AI button now reads **Rewrite translations with AI**, which rewrites what is already there rather than filling in the gaps.
...
- [Variables](/reference/email-builder/variables) explains the badges that survive translation.
