...
<img src="/images/forms-builder/21-object-button.png" alt="The form builder toolbar with the cursor on the third drawer button and its Show objects tooltip" width="650" />
...
<img src="/images/forms-builder/23-objects-panel-linked.png" alt="The Objects panel with Vacancy expanded to its Label, Key, Required, and Remove controls, Match below it, and the Available objects list underneath" width="460" />
## What linking an object gives you

Every field on the linked record becomes a variable. You can write it into any text setting the form has, including the question, the description, a placeholder, and an option label, so a question can open with the job title rather than "this role". In the builder the variable shows as a badge such as **Vacancy: Job Title**.

The variable list in the toolbar shows them all. Each one is named after the object's label and addressed by its key.

<img src="/images/forms-builder/33-object-variables.png" alt="The Variables drawer filtered to the job key, listing Vacancy: Job Title and the other fields of the linked vacancy with their variable names" width="460" />
...
- **Remove** unlinks the object. Its variables stop resolving and every badge that used one turns orange, so check every question that used one.
