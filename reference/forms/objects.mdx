...
description: "Linking records such as a vacancy or a match to a HireData form, the settings each link carries, and why the first object decides which records it serves."
...
Every field on the linked record becomes a variable. You can write it into any text setting the form has, including the question, the description, a placeholder, and an option label, so a question can open with the job title rather than "this role". In the builder the variable shows as a badge reading the object's label and the field's name, such as **Vacancy: Job Title**. Hover it to see the value and any modifiers.
...
- **Remove** unlinks the object. Its variables stop resolving. Every badge that used one turns orange and its popover reads **Variable is not found**, so check every question that used one.
...
The form's own page is more forgiving. Open a public form with nothing supplying the record and it still loads. Every variable from that object resolves to nothing, unless it carries a `default` modifier. See [Fallback](/variables/modifiers#fallback).
