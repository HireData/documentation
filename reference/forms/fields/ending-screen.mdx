...
Endings live in their own **Endings** group at the bottom of the field list, separate from the questions. A form can hold several of them, which is the point: a logic rule can send a strong candidate to one ending and everyone else to another.
...
None. The **Logic** tab lists the endings at the bottom but gives them no **Add Rule** button, because an ending is where the form stops. Rules on the questions above point at endings, not the other way round.
