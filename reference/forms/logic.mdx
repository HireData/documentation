...
title: "Logic"
sidebarTitle: "Logic"
description: "Conditional logic in a HireData form. How a rule reads, which operators each field type offers, the branching patterns worth copying, and what to check when a rule does not fire."
keywords: ["conditional logic", "branching", "skip logic", "jump", "add rule", "add condition", "all other cases go to", "always go to", "operators"]
---
...
The tab lists fields in form order, so the **Endings** divider falls wherever the first ending sits. That can be halfway down, with questions still to come underneath. The **Content** tab gathers endings at the bottom instead, so the two lists rarely match.

![The Logic tab showing a question with Always go to, then the Endings divider, an ending with no Add Rule button, and the next question underneath](/images/forms-builder/32-logic-endings-divider.png)

An ending has no **Add Rule** button and no footer target. Nothing follows it.
...
Three options that all lead to the same follow-up need one rule with **is one of**, not three rules.
...
Work down this list before you go looking for a bug.

- **A rule above it matched first.** HireData stops at the first match. Move the narrow rule above the broad one.
- **The rule needed both conditions.** Conditions inside one rule are joined with and. Split them into two rules to get or.
...
- **The target sends people backwards.** A rule can point at a question already behind the respondent, and nothing stops them going round the same questions twice. Try any backwards jump in **Interactive Preview** before you publish.
