...
A rule reads `if <field> <operator> <value> then go to <target>`. **Add Condition** puts a second condition in the same rule, and a rule fires only when every one of its conditions matches. To offer alternatives, add a second rule instead. A field can carry as many rules as it needs.

HireData checks a field's rules from top to bottom and follows the first one whose conditions all match. When none match, it follows the footer target. Without a footer target, the respondent continues to the next field in the list.
...
![Test & Evaluate after an ideal-candidate run, showing Meets criteria, 100% High, Availability at 66.67% contribution, the summary, and the generated fields](/images/forms-builder/30-test-and-evaluate.png)
...
|---|---|---|
| Short Text and Long Text | Text | text |
| Email | Contact Info | text |
| Phone Number | Contact Info | text |
| Website | Contact Info | text |
| Date | Temporal | a date and time |
| Time | Temporal | a date and time |
| Datetime | Temporal | a date and time |
| Yes/No | Choice | Yes or No |
| Multiple Choice and Dropdown | Choice | the chosen options |
| Number | Numeric | a number |
| Currency | Numeric | a number |
| Net Promoter Score and Opinion Scale | Rating | a point on the scale, 0 to 10 for Net Promoter Score |
| File Upload | Other | the uploaded files |
| Ending Screen | Ending | nothing |
| Redirect URL | Ending | nothing |

The type also decides which operators the [Logic tab](#logic-tab) offers.

| Field types | Operators |
|---|---|
| Short Text, Long Text, Email, Phone Number, Website | has response, is equal to, is not equal to, starts with, ends with, contains, does not contain |
| Multiple Choice, Dropdown | has response, is equal to, is not equal to, is one of, is not one of |
| Yes/No | has response, is equal to, is not equal to |
| Number, Currency, Net Promoter Score, Opinion Scale | has response, is equal to, is not equal to, is greater than, is less than, is greater than or equal to, is less than or equal to |
| Date, Time, Datetime, File Upload | has response |
| Ending Screen, Redirect URL | no rules |

Every field type except the two endings can feed the AI evaluators on the [Evaluation tab](#evaluation-tab): Criteria, Scoring, and Skills.
...
<Warning>
  A bug in the web form blocks these answers. Answer No, leave a toggle off, or enter 0 on a required field and the respondent gets stuck. **Next** does nothing, no message appears, and any rule on that field does not fire. Until the fix ships, keep Yes/No fields optional and set a **Minimum Value** of 1 on required numeric fields.
</Warning>
