...
description: "A tour of the HireData form builder: its three tabs, the toolbar drawers, all 15 field types, and the settings every field shares."
keywords: ["form builder", "form fields", "field types", "form theme", "form translations", "form templates", "make form public", "always include", "hidden field", "required field"]
---

A form is a set of questions someone answers on the web or in a WhatsApp conversation. Open the **Forms** page from **Settings** and click **New Form** to start one, then build it in the form builder.
...
The **+** button opens the **Add Content** palette. It groups the 18 palette entries by category and repeats the six most used ones under **Recommended**. **Find a field type** searches the list.
...
The **Logic** tab decides where a respondent goes next. Every field gets an **Add Rule** button and a footer target that reads **All other cases go to** once the field has a rule, or **Always go to** when it has none.

A rule reads `if <field> <operator> <value> then go to <target>`. HireData checks a field's rules from top to bottom and follows the first one whose conditions all match. When none match, it follows the footer target. Without a footer target, the respondent continues to the next field in the list.
...
The **Evaluation** tab turns a finished response into something a recruiter can act on. It holds five evaluators, each with a switch:

- **Summary** writes a short internal summary of the response.
- **Criteria** checks selected answers against must-have and nice-to-have requirements.
- **Scoring** awards points for selected answers and converts the total into a percentage and a rating.
- **Skills** works out how strongly the answers point to each skill you listed, as a percentage.
- **Generated fields** asks the AI for extra values you define, such as a preferred contact channel.

Switch an evaluator on here and it also appears on every field card, so you can enable it per field without leaving the **Content** tab.
...
**Interactive Preview** walks the web form, running the same logic the public form runs. **Conversational Preview** shows the chat version. **Test & Evaluate** fills in a set of answers and runs the evaluators over them, so you can check a rule before a real respondent hits it.

**Test & Evaluate** starts empty. **Autofill with AI** invents a plausible response, and **Choose a test scenario** offers **Random answers**, **Ideal candidate**, **Borderline candidate**, **Unqualified candidate**, and **Partially completed**. The results appear underneath: the criteria verdict, the score and its rating, the strongest skill, the summary, and any generated fields. **Reset answers** clears them.

![Test & Evaluate showing an ideal-candidate run: Meets criteria, 100% High, Availability at 71.43% contribution, with the summary and generated fields below](/images/forms-builder/30-test-and-evaluate.png)
...
**Save** stores your changes. The eye button next to it opens the form in a new window.
...
- **Show objects** links records such as a vacancy or a match to the form. See [Objects and data sources](#objects-and-data-sources).
...
The **Add Content** palette shows 18 entries, which make 15 field types. Short Text and Long Text, Multiple Choice and Dropdown, and Net Promoter Score and Opinion Scale are each one type with two settings, switched in the field card's footer.

| Field type | Group | Stores | Logic operators |
|---|---|---|---|
| Short Text and Long Text | Text | text | text |
| Email | Contact Info | text | text |
| Phone Number | Contact Info | text | text |
| Website | Contact Info | text | text |
| Date | Temporal | a date and time | has response |
| Time | Temporal | a date and time | has response |
| Datetime | Temporal | a date and time | has response |
| Yes/No | Choice | Yes or No | Yes/No |
| Multiple Choice and Dropdown | Choice | the chosen options | choice |
| Number | Numeric | a number | numeric |
| Currency | Numeric | a number | numeric |
| Net Promoter Score and Opinion Scale | Rating | a point on the scale, 0 to 10 for Net Promoter Score | numeric |
| File Upload | Other | the uploaded files | has response |
| Ending Screen | Ending | nothing | rules not available |
| Redirect URL | Ending | nothing | rules not available |

Every field that takes rules offers **has response**, which is true once the field has any answer. Text and contact fields add **is equal to**, **is not equal to**, **starts with**, **ends with**, **contains**, and **does not contain**. Yes/No adds **is equal to** and **is not equal to**. Multiple Choice and Dropdown add those two, plus **is one of** and **is not one of**. Numeric and rating fields add **is equal to**, **is not equal to**, **is greater than**, **is less than**, **is greater than or equal to**, and **is less than or equal to**. Date, Time, Datetime, and File Upload add none, and the two endings take no rules.

Every field type except the two endings can take part in Criteria, Scoring, and Skills.
...
![An Email field card showing Question, Description, Image, Placeholder, the length and pattern settings, both auto replies, the three evaluator switches, and the footer button reading Required](/images/forms-builder/02-question-card-common-settings.png)
...
An empty text box, an empty number box, no option chosen, and no file attached all count as no answer, and the web form shows a message under the question, such as "This field is required and cannot be empty."

Answering No, turning a toggle off, and entering 0 are real answers and are meant to satisfy a required field.

<Warning>
  The web form at forms.hiredata.com currently gets this wrong. A respondent who answers No on a required Yes/No field, leaves a required toggle off, or enters 0 in a required Number, Currency, or rating field clicks **Next** and nothing happens. No message appears. The answer is saved, but the form does not advance, and any logic rule on that field does not fire. Until this is fixed, keep Yes/No fields optional, and set a **Minimum Value** of 1 on required numeric fields where 0 is not a meaningful answer.
</Warning>
...
**Show objects** in the toolbar links records to the form. Link a vacancy and its fields become variables you can write into any question. The first object in the list is the form's main object, and it decides which records you can configure the form for. See [Configuring a form per record](/content/configuring-a-form-per-record).

A data source is a saved list your workspace can reuse. Point a Multiple Choice or Dropdown field at one with **Add Data Source** and its options come from that list instead of options you type. The source belongs to the workspace, so editing one changes every field that uses it.

## Theme and publishing {#theme-and-publishing}

The **Settings** drawer controls how the form looks and who can reach it.

![The Settings drawer with Make form public, Logo, Alignment, Font Family, Text Color, Scale, the Input and Background sections, Button Text, and Border radius](/images/forms-builder/26-settings-drawer.png)
...
![The New Translation panel with English selected, the Create AI translation button, and the Settings and Fields sections showing each source string above an empty box](/images/forms-builder/27-translation-editor.png)
