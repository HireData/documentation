...
- **Show variable list** lists the variables the form can use, each with its type and current value, and a **Search** box to filter them. A variable written into a question shows as a badge with the field's display name. See [Variables](/variables/introduction).
...
Text settings such as **Question**, **Description**, and the placeholders accept variables, so a question can address the respondent by name or mention the vacancy. Each variable shows as a badge reading the field's display name, such as **Vacancy: Job Title**. Hover it for the value and any modifiers. A key the form does not know turns the badge orange with the note **Variable is not found**. Some variables only become available after you link their object to the form, see [Objects and data sources](#objects-and-data-sources). For the syntax, see [Variables](/variables/introduction).

<img src="/images/forms-builder/37-variable-popover.png" alt="The Description box of a form question with four badges, and a popover open above it headed Name with cards reading Input NPS required question, Lowercase, Capitalise words, Use a fallback Your form, and Output Nps Required Question" width="573" />
