...
Either way the field is stored as a `{{field_key}}` expression and shown as a badge reading the field's display name, such as **Form: Id**. Hover the badge and a popover shows the field's default value and any [modifiers](/variables/modifiers). A key the automation no longer offers shows an orange badge reading **Variable is not found**.
...
- A value bound to an automation field, unless that field happens to have a default value. The badge is resolved against that default, and most fields carrying run data don't have one. The badge's popover shows the same default under **Value**.
