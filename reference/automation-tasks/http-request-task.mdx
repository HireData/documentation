...
An automation can call an external API by itself. The **HTTP Request** task sends a request to an address you choose, with the authentication that endpoint expects. It makes the response available to the tasks that follow once you describe what it contains. You don't need a developer, and you don't need anything sitting in between.
...
<Tabs>
  ...
  <Tab title="Upload file">
    Select the document from your computer, for when what you were sent isn't text you can copy. The route is labelled "A specification or collection file".
  </Tab>
  <Tab title="From URL">
...
![The endpoint chooser, after importing a specification with several operations](/images/http-request/endpoint-picker.png)
...
![The Headers tab, with the Name combobox suggesting known header names](/images/http-request/headers-tab.png)
...
Either way the field is stored as a `{{field_key}}` expression and shown as a badge with the field's name, such as **Form: Id**. Hover it for the field's default value and any [modifiers](/variables/modifiers). A key the automation no longer offers turns the badge orange.

Fields can go into the URL, the parameters, the headers, and the body. Any part of the request can change from run to run.

![The field picker open on a Value, listing fields from the automation](/images/http-request/field-picker.png)

## Authentication

The **Auth** tab is a segmented control with five options: `None`, `API key`, `Bearer token`, `Basic auth`, and `OAuth2`. Select the one the endpoint's owner told you to use and its fields appear.
...
![A later task's field picker, showing "Get brand context by domain: meta.canonical_name"](/images/http-request/response-field-in-later-task.png)
...
- A value bound to an automation field, unless that field happens to have a default value. The badge is resolved against that default, and most fields carrying run data don't have one.
...
![The Test dialog before Send, with the Path inputs empty and the Body inputs carrying the values configured on the task](/images/http-request/test-dialog-before-send.png)
...
![A completed HTTP Request in a run: both sections and the chip row of auth type, timeout, and retries](/images/http-request/task-in-a-run.png)
