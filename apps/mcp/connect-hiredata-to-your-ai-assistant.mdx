---
title: "Connect HireData to your AI assistant"
sidebarTitle: "Connect via MCP"
description: "Connect ChatGPT, Claude, or Gemini to HireData in minutes: add the HireData MCP connector, sign in with your own account, and let your AI assistant work safely in your workspaces."
---

_HireData has an official MCP (Model Context Protocol) connection. Any AI assistant that supports MCP can connect to HireData with your own account: it sees only the workspaces you grant, works with the same permissions you have in the app, and every request is verified and recorded in the activity log. This guide shows you how to set up the connection in ChatGPT, Claude, and Gemini._
...
On Team and Enterprise plans, an organization owner first adds the connector in **Organization settings** \> **Connectors**; after that, each member connects it from their own **Settings** \> **Connectors**.
...
In a conversation, click the **\+** button, choose **Connectors**, and make sure **HireData** is enabled. Then just ask — for example: _"Could you check my latest automation and its stats?"_

![Claude using the HireData connector to review automation statistics](/images/mcp/claude-example-chat.png)
...
- "Could you check my latest automation and its stats?"
