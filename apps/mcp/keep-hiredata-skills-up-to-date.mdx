...
related: ["/apps/mcp/check-for-new-skills-automatically", "/apps/mcp/install-hiredata-plugin", "/apps/mcp/hiredata-skills", "/apps/mcp/install-hiredata-skills-with-a-prompt"]
---
...
If you'd rather not remember to check, don't. Claude and ChatGPT can both run a prompt on a schedule, so one of them checks the skills library every morning and only speaks up when something has actually moved — see [check for new skills automatically](/apps/mcp/check-for-new-skills-automatically) for the copy-paste prompt per assistant.

No scheduled tasks on your plan? Follow the repository instead: open [github.com/HireData/skills](https://github.com/HireData/skills), click **Watch**, and choose what you want to hear about — you need a GitHub account. If you use an RSS reader, `https://github.com/HireData/skills/commits/main.atom` gives you the same changes as a feed. New skills are also announced with other product news at [hiredata.com](https://www.hiredata.com/).
