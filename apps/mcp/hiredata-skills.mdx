...
sidebarTitle: "Skills"
description: "Overview of the six HireData skills that guide ChatGPT, Codex, and Claude through planning activations, creating workflows, and writing actionable issue reports."
keywords: ["HireData skills", "AI skills", "plan activation", "create triggers", "create forms", "AI email templates", "AI WhatsApp templates", "AI issue reports", "HireData plugin", "MCP"]
...
HireData currently publishes six skills. Each gathers the available context, shows you a complete draft or preview, and only creates or changes something after you approve that exact action. New triggers stay inactive, forms stay unpublished where drafts are supported, and nothing is sent or activated as a side effect.
...
| [Write issue reports](#write-issue-reports) | Turn evidence and requests into clear, actionable reports for HireData |
...
Turns a bug, customer request, prototype, recording, meeting note, or technical prompt into a structured HireData issue report. The skill preserves evidence, separates facts from assumptions, chooses the smallest useful issue structure, and makes missing product or technical decisions visible. Customers and partners get a report they can send to HireData without needing internal Linear access. Authorized HireData users can also refine or update live issues when they explicitly request it.

Try:

- "Turn these reproduction steps, screenshot, and automation URL into a HireData-compatible bug report. Draft only."
...
<CardGroup cols={2}>
  <Card title="Install the plugin" icon="puzzle-piece" href="/apps/mcp/install-hiredata-plugin">
    Get all six skills plus the MCP connection in ChatGPT, Codex, or Claude.
  </Card>
