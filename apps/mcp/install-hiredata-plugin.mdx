...
| MCP only | Live HireData tools without the five skills | You use Gemini, another MCP client, or only need tool access |
...
<Warning>
  ChatGPT on the web cannot install the plugin. It cannot add a marketplace, cannot add MCP servers, and does not show plugins you installed in the desktop app — so HireData will not appear there after a desktop install. It can upload skills from your computer. Use the desktop app for the full plugin.
</Warning>
...
- **HireData does not appear in ChatGPT on the web after a desktop install:** the web app cannot add marketplaces or MCP servers, and does not show plugins installed in the desktop app. Use the desktop app.
