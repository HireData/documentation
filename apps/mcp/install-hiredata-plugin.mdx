...
description: "Install the HireData plugin in ChatGPT, Codex, or Claude to add six recruitment and issue-reporting skills plus the MCP connection in one step."
...
The plugin includes six skills. See [HireData skills for your AI assistant](/apps/mcp/hiredata-skills) for what each one does and example prompts:
...
- [**Write issue reports**](/apps/mcp/write-better-hiredata-issue-reports-with-ai) — turn evidence, requests, and prototypes into clear reports for HireData without inventing internal metadata or implementation details.
...
| --- | --- | --- |
| HireData plugin | Six HireData skills and the MCP connection | You use ChatGPT desktop, Codex, or Claude and want both HireData knowledge and tools |
| MCP only | Live HireData tools without the six skills | You use Gemini, ChatGPT on the web, another MCP client, or only need tool access |
...
This works in ChatGPT, Gemini, Microsoft Copilot, or any assistant that can read public web pages, and pairs well with the [manual MCP connection](/apps/mcp/connect-hiredata-to-your-ai-assistant). The six folder names are `hiredata-plan-activation`, `hiredata-create-triggers`, `hiredata-create-forms`, `hiredata-create-email-templates`, `hiredata-create-whatsapp-templates`, and `hiredata-create-linear-issues`.
