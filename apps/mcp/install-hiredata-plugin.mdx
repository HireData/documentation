...
description: "Install the HireData plugin in ChatGPT, Codex, or Claude to add nine recruitment, diagnostic, and issue-reporting skills plus the MCP connection in one step."
...
- [**Diagnose automations**](/apps/mcp/hiredata-skills#diagnose-automations) — establish what actually happened to a workflow, from the evidence, before proposing a repair.
- [**Diagnose email performance**](/apps/mcp/hiredata-skills#diagnose-email-performance) — explain a drop in open rates, click rates, or deliverability, and what to do about it.
...
- [**Map the staffing lifecycle**](/apps/mcp/hiredata-skills#map-the-staffing-lifecycle) — staffing and ATS domain knowledge that helps the other skills read a request correctly.
...
| --- | --- | --- |
| HireData plugin | Nine HireData skills and the MCP connection | You use ChatGPT desktop, Codex, or Claude and want both HireData knowledge and tools |
| MCP only | Live HireData tools without the nine skills | You use Gemini, ChatGPT on the web, another MCP client, or only need tool access |
...
- "Use HireData to work out why our interview reminder automation went quiet last week. Diagnose only, change nothing."
...
Can't install the plugin, either because your assistant has no plugin support or because your organisation blocks the marketplace? The skills are plain, public files, so you can let your AI assistant fetch and package them for you instead.
...
This works in ChatGPT, Gemini, Microsoft Copilot, or any assistant that can read public web pages, and pairs well with the [manual MCP connection](/apps/mcp/connect-hiredata-to-your-ai-assistant). The nine folder names are `hiredata-plan-activation`, `hiredata-create-triggers`, `hiredata-create-forms`, `hiredata-create-email-templates`, `hiredata-create-whatsapp-templates`, `hiredata-diagnose-automations`, `hiredata-diagnose-email-performance`, `hiredata-create-linear-issues`, and `hiredata-map-staffing-lifecycle`.
