...
sidebarTitle: "Profile pictures"
description: "Understand how HireData selects user avatars from Carerix attachments, the priority order applied, and when photos, logos, or initials are displayed."
---
...
HireData also evaluates the attachment label. Matching is case-insensitive, so any casing variation (for example, `PasFoto`, `PASfoto`, `EmailLogo`) is supported.

If multiple valid attachments exist, HireData evaluates them in a fixed priority order. Within that order, HireData considers the most recently modified attachment first.
...
HireData gives first priority to an attachment where:

- Type value is `emaillogo`
- Label is `emailpasfoto`

If such an attachment exists and contains usable content, HireData uses it as the avatar.
...
If found and valid, HireData uses this image.
...
HireData displays an image as the avatar when it finds a **matching attachment** based on the priority above and a valid image is available to use as the user's avatar.

## When are initials displayed?

HireData shows initials when:
