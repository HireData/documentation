...
description: "Switch individual form questions on or off per vacancy in HireData, so one pre-screening form fits every client and role without duplicates."
...
![The Forms tab of a vacancy record, with a tab per linked form and a table of questions with Available and Criteria columns, the first question carrying two variable badges](/images/record-forms/02-record-forms-configuration.png)
...
### Variables in the question list

A question that uses a variable shows it as a badge reading the field's display name, such as **Vacancy: Job Title**. Hover the badge and a popover shows the value this record supplies. A variable with a `default` modifier shows an **Input** card with the record's value, a **Use a fallback** card with the fallback text, and an **Output** card with what this record's respondents read.

<img src="/images/record-forms/05-record-forms-variable-popover.png" alt="The Forms tab of a vacancy record with a popover open under the Vacancy: Job Title badge, showing Input General Vacancy, Use a fallback position you applied for, and Output General Vacancy" width="942" />

When the record has no value for the field, the **Input** card reads **No value provided** and the **Output** card shows the fallback. Without a fallback the question shows a gap where the value would be, so use this tab to check a vacancy before you send its form. A key the form no longer knows, for example after its object was removed, shows an orange badge reading **Variable is not found**. See [Variables](/variables/introduction).
