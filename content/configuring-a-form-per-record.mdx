...
A question that uses a variable shows it as a badge such as **Vacancy: Job Title**. Hover the badge to see the value this record supplies and, for a variable with a fallback, what its respondents will read.
...
When the record has no value for the field, the popover reads **No value provided** and shows the fallback as the result. Without a fallback the question goes out with a gap, so use this tab to check a vacancy before you send its form. See [Variables](/variables/introduction).
